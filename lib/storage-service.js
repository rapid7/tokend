...
class StorageService {
  ...
  constructor(options) {
    ...
    this.defaultSecret = this._getLeaseManager(DEFAULT_SECRET, TokenProvider);
  }
   ...
   *
   */
  lookup(secret, ProviderType, callback) {
    const manager = this._getLeaseManager(secret, ProviderType);
   ...
   *
   */
  _getLeaseManager(secret, ProviderType) {
    const paths = secret.split('/').filter((n) => !!n);

    // TODO: Use array destructuring when Node supports it
    // const api = paths[0];
    // const type = paths[1];
    // const token = paths[2];
    const path = paths[3] || '';
    ...
    const t = (!!this.defaultSecret && this.defaultSecret.status === STATUS.READY) ? this.defaultSecret.data.token : '';

    // Otherwise, provision a new LeaseManager for the secret
    // TODO: get provider configuration (options) using secret
    const options = {};
    const provider = new ProviderType(path, t, options);
