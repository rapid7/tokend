...
class StorageService {
  ...
  _getLeaseManager(token, secret, ProviderType) {
    const secretID = this._getSecretID(token, secret, ProviderType);
  ...
  /**
   * Gets a unique ID for the given token, secret, and ProviderType
   *
   * @param {String} token
   * @param {String} secret
   * @param {*} ProviderType
   ...
   * @private
   */
  _getSecretID(token, secret, ProviderType) {
    if (ProviderType.hasOwnProperty('getSecretID')) {
      return `/${ProviderType.prototype.constructor.name}/${ProviderType.getSecretID(token, secret)}`;
    }
    return `/${ProviderType.prototype.constructor.name}/${token}/${secret}`;
  }
  ...
  _setEvents(manager) {
    ...
    manager.on('ready', () => {
      Log.log('INFO', 'Manager is ready', {
        ...
        renewable: manager.renewable
      });

      if (manager.renewable === false) {
        this._managers.delete(manager.name);
      }
    });
