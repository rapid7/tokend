...
const getDefaultLeaseTTL = require('../utils/tune').getDefaultLeaseTTL;
...
class TokenProvider {
  ...
  renew() {
    ...
    return this._client.prepare(this.token)
      .then(getDefaultLeaseTTL.bind(this, this._client, this.token, 'sys'))
      .then((response) => {
        this.default_token_ttl = response;
      })
      .then(this._client.renewToken.bind(this._client, {
        id: this.token,
        token: this.token,
        body: {increment: this.default_token_ttl}
      }, null))
      .then((data) => {
        this.data = {
