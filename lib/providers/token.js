...
class TokenProvider {
  ...
  constructor(options) {
    ...
    if (opts.warden) {
      this.warden.hostname = opts.warden.host;
      this.warden.port = opts.warden.port;
    }
  ...
  _sendDocument() {
    return new Promise((resolve, reject) => {
      Promise.all(METADATA_ENDPOINTS.map((path) => promisify((done) => this._metadata.request(path, done))))
      .then((data) => {
        resolve(this._post({
